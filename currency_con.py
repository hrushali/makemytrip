...
def convert_currency(qty, from_currency, to_currency):
    try:
        qty = float(round(float(qty), 2))
        amount = round(qty * fx[to_currency] / fx[from_currency], 2)
        return f"{qty} {from_currency} = {amount} {to_currency}"
    except KeyError:
...
class CurrencyConverterApp:
    def __init__(self, root):
        ...
        self.qty_label = ttk.Label(root, text="Amount:")
        self.qty_entry = ttk.Entry(root)

        self.from_currency_label = ttk.Label(root, text="From Currency:")
        self.from_currency_combobox = ttk.Combobox(root, values=[currency.split(":")[0] for currency in currencies])

        self.to_currency_label = ttk.Label(root, text="To Currency:")
        self.to_currency_combobox = ttk.Combobox(root, values=[currency.split(":")[0] for currency in currencies])

        self.result_label = ttk.Label(root, text="Result:")

        self.convert_button = ttk.Button(root, text="Convert", command=self.convert)

        self.qty_label.grid(row=0, column=0, padx=10, pady=10, sticky="E")
        self.qty_entry.grid(row=0, column=1, padx=10, pady=10)

        self.from_currency_label.grid(row=1, column=0, padx=10, pady=10, sticky="E")
        self.from_currency_combobox.grid(row=1, column=1, padx=10, pady=10)

        self.to_currency_label.grid(row=2, column=0, padx=10, pady=10, sticky="E")
    ...
    def convert(self):
        ...
        result = convert_currency(qty, from_currency, to_currency)
